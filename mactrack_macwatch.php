...
function form_save()
{
    if (isset_request_var('save_component_macw') && isempty_request_var('add_dq_y')) {
        $mac_id = api_mactrack_macw_save(
            get_nfilter_request_var('mac_id'),
            get_nfilter_request_var('mac_address'),
            get_nfilter_request_var('name'),
            get_nfilter_request_var('ticket_number'),
            get_nfilter_request_var('description'),
            get_nfilter_request_var('notify_schedule'),
            get_nfilter_request_var('email_addresses')
        );
...
function form_actions()
{
    ...
    foreach ($_POST as $var => $val) {
        if (preg_match('/^chk_([0-9]+)$/', $var, $matches)) {
            ...
            $macw_info = db_fetch_cell_prepared(
                'SELECT name
...
function mactrack_macw_edit()
{
    ...
    if (!isempty_request_var('mac_id')) {
        $mac_record = db_fetch_row_prepared(
            'SELECT *
			FROM mac_track_macwatch
			WHERE mac_id = ?',
...
function mactrack_macw()
{
    ...
    if (cacti_sizeof($macw)) {
        foreach ($macw as $mac) {
            ...
            form_selectable_cell('0000-00-00 00:00:00' == $mac['date_first_seen'] ? __('N/A', 'mactrack') : $mac['date_first_seen'], $mac['mac_id']);
            form_selectable_cell('0000-00-00 00:00:00' == $mac['date_last_seen'] ? __('N/A', 'mactrack') : $mac['date_last_seen'], $mac['mac_id']);
            form_checkbox_cell($mac['name'], $mac['mac_id']);

            form_end_row();
        }
    } else {
        echo '<tr><td colspan="'.$columns.'"><em>'.__('No Mactrack Watched Macs Found', 'mactrack').'</em></td></tr>';
    }
