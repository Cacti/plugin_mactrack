...
function mactrack_filter_table() {
	...
	for ($bwpercent = 10; $bwpercent < 100; $bwpercent += 10) {
		?><option value='<?php print $bwpercent; ?>' <?php if (isset_request_var('bwusage') && (get_request_var('bwusage') == $bwpercent)) {?> selected<?php }?>> >=<?php print $bwpercent; ?>%</option><?php
	}
	?>
						</select>
					</td>
	...
	if (get_request_var('site_id') != -1) {
		$sql_where .= ' WHERE mac_track_devices.site_id=' . get_filter_request_var('site_id');
	} else {
		$sql_where  = '';
	}
	...
	if (cacti_sizeof($types)) {
		foreach ($types as $type) {
			...
			if (get_request_var('device_type_id') == $type['device_type_id']) {
				print ' selected';
			} print '>' . html_escape($type['device_type']) . '</option>';
		}
	}
	?>
						</select>
	...
	if (get_request_var('site_id') != -1) {
		$sql_where .= ($sql_where != '' ? ' AND ' : 'WHERE ') . 'site_id=' . get_filter_request_var('site_id');
	}

	if (get_request_var('device_type_id') != '-1') {
		$sql_where .= ($sql_where != '' ? ' AND ' : 'WHERE ') . 'device_type_id=' . get_filter_request_var('device_type_id');
	}
