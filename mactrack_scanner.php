...
if (cacti_sizeof($parms)) {
    ...
    echo "ERROR: You must supply input parameters\n\n";
    display_help();
...
if (!$device_id) {
    echo "ERROR: missing id (host_id) parameter\n\n";
    display_help();
...
if (0 == cacti_sizeof($device_types)) {
...
if (valid_snmp_device($device)) {
    ...
    if (isset($device_type) && cacti_sizeof($device_type) > 0) {
        ...
        if ((DEVICE_HUB_SWITCH == $device['scan_type'])
                || (DEVICE_SWITCH_ROUTER == $device['scan_type'])) {
        ...
        if ((DEVICE_SWITCH_ROUTER == $device['scan_type'])
            || (DEVICE_ROUTER == $device['scan_type'])) {
...
function display_version()
{
    ...
    echo 'Network Mactrack, Version '.$info['version'].', '.COPYRIGHT_YEARS."\n";
}
...
function display_help()
{
    ...
    echo "-id=host_id   - the mac_track_devices host_id to scan\n";
    echo "-d | --debug  - Display verbose output during execution\n";
    echo "-w | --web    - Display web compatible output during execution\n";
    echo "-t            - Test mode, don't log a process id and interfere with system\n";
