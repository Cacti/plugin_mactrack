...
function sync_mactrack_to_cacti($mt_device)
{
    ...
    if ((3 == read_config_option('mt_update_policy', true))
        && ($mt_device['host_id'] > 0)) {
        ...
        if (cacti_sizeof($cacti_device)) {
            ...
            api_device_save(
                $cacti_device['id'],
                $cacti_device['host_template_id'],
                $cacti_device['description'],
                $cacti_device['hostname'],
                $mt_device['snmp_readstring'],
                $mt_device['snmp_version'],
                $mt_device['snmp_username'],
                $mt_device['snmp_password'],
                $mt_device['snmp_port'],
                $mt_device['snmp_timeout'],
                $cacti_device['disabled'],
                $cacti_device['availability_method'],
                $cacti_device['ping_method'],
                $cacti_device['ping_port'],
                $cacti_device['ping_timeout'],
                $cacti_device['ping_retries'],
                $cacti_device['notes'],
                $mt_device['snmp_auth_protocol'],
                $mt_device['snmp_priv_passphrase'],
                $mt_device['snmp_priv_protocol'],
                $mt_device['snmp_context'],
                $mt_device['snmp_engine_id'],
                $mt_device['max_oids']
            );
...
function sync_cacti_to_mactrack($device)
{
     ...
     * (aka: has the device been saved successfully) */
    if ((2 == read_config_option('mt_update_policy', true)) && ($device['id'] > 0)) {
        ...
        if (is_array($mt_device) && $mt_device) {
            ...
            mactrack_debug(__('Mactrack Device: (%s) successfully updated', $mt_device['device_id'], 'mactrack'));
        }
    }
...
function mactrack_device_action_prepare($save)
{
    ...
    if (isset($save['drp_action']) && 'plugin_mactrack_device' == $save['drp_action']) {
        // find out which (if any) hosts have been checked, so we can tell the user
        if (isset($save['host_array'])) {
            // list affected hosts
            echo '<tr>';
            echo "<td colspan='2' class='textArea'>"
            ...
            echo '<tr>';
            echo "<td colspan='2' class='textArea'>"
                .'<p>'.__('We will use these devices SNMP options for the Mactrack Device as well.', 'mactrack').'</p>'
...
function mactrack_device_action_execute($action)
{
    ...
    if ('plugin_mactrack_device' == $action) {
        // find out which (if any) hosts have been checked, so we can tell the user
