...
/**
 * Scans the dot1d OID tree of a Dell switch for all switch
 ...
 * dot1d bridge-port SNMP table for a Dell switch, optionally storing
 * results to the database. Foundational function reused by vendor
 * data collection functions; originally copied from the default
 * dot1q function in mactrack_functions.php and modified for this
 * vendor's non-numeric SNMP value quirks.
 *
 * @param array $site            The site record the device belongs to.
		...
		if (cacti_sizeof($new_port_key_array)) {
			...
			if (cacti_sizeof($new_port_key_array)) {
				foreach ($new_port_key_array as $key => $port) {
					...
					$new_port_key_array[$key]['vlan_name'] = mactrack_arr_key($vlan_names, $new_port_key_array[$key]['vlan_id']);
				}
			}
...
/**
 * Normalizes a Dell switch's raw bridge-port MAC address value (which
 * may be an ASCII-encoded string using '.' or '-' separators) into a
 * plain uppercase hex MAC address with no separators.
 *
 * @param string $mac_address The raw MAC address value from SNMP.
 *
 * @return string The normalized uppercase hex MAC address, or
 *                'NOT USER' when the input is empty.
 */
