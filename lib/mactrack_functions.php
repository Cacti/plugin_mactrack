...
function mactrack_rebuild_scanning_funcs()
{
    ...
    if (isset($mactrack_scanning_functions)) {
        foreach ($mactrack_scanning_functions as $scanning_function) {
            db_execute_prepared(
                'REPLACE INTO mac_track_scanning_functions
				(scanning_function, type)
				VALUES (?, ?)',
                [$scanning_function, 1]
            );
...
function find_scanning_function(&$device, &$device_types)
{
    ...
    if (cacti_sizeof($device_types)) {
        foreach ($device_types as $device_type) {
            ...
            if ((true == $sysObjectID_match) && (true == $sysDescr_match)) {
...
function port_list_to_array($port_list, $delimiter = ':')
{
    ...
    if ('-1' == read_config_option('mt_ignorePorts_delim')) {
        // find the delimiter
...
function build_InterfacesTable(&$device, &$ifIndexes, $getLinkPorts = false, $getAlias = false)
{
    ...
    if (cacti_sizeof($ifAdminStatus)) {
        foreach ($ifAdminStatus as $key => $value) {
            if (substr_count(strtolower($value), 'up') || ('1' == $value)) {
                $ifAdminStatus[$key] = 1;
            } else {
                $ifAdminStatus[$key] = 0;
            }
    ...
    foreach ($ifIndexes as $ifIndex) {
        ...
        if (!isset($db_interface[$ifIndex])) {
            if (isset($ifOperStatus[$ifIndex]) && 1 == $ifOperStatus[$ifIndex]) {
            ...
            if (0 == $db_interface[$ifIndex]['ifOperStatus']) { // interface previously not up
                if (isset($ifOperStatus[$ifIndex]) && 1 == $ifOperStatus[$ifIndex]) {
                    ...
                    if ('0000-00-00 00:00:00' == $db_interface[$ifIndex]['last_down_time']) {
                    ...
                    if ('0000-00-00 00:00:00' == $db_interface[$ifIndex]['last_up_time']) {
                        $last_up_time = date('Y-m-d H:i:s');
                    }
                }
            } else {
                if (isset($ifOperStatus[$ifIndex]) && 0 == $ifOperStatus[$ifIndex]) {
        ...
        if (isset($ifLastChange[$ifIndex]) && false !== strpos($ifLastChange[$ifIndex], ':')) {
    ...
    if ('' != $insert_vals) {
        ...
        db_execute_prepared(
            'DELETE FROM mac_track_interfaces
			WHERE present=0
			AND device_id = ?',
            [$device['device_id']]
        );
...
function mactrack_timetics_to_seconds($timetics)
{
    ...
    if (4 == cacti_sizeof($parts)) {
        $time += $parts[0] * 86400;
        $time += $parts[1] * 3600;
        $time += $parts[2] * 60;
        $time += round($parts[3], 0);
    } elseif (3 == cacti_sizeof($parts)) {
        $time += $parts[0] * 3600;
        $time += $parts[1] * 60;
...
function mactrack_find_host_graphs($device_id, $host_id)
{
        ...
        foreach ($local_data_ids as $local_data_id) {
            $local_graph_ids = array_rekey(
                db_fetch_assoc_prepared(
                    'SELECT DISTINCT gtg.local_graph_id AS id, gtg.graph_template_id
    ...
    if ($found) {
        ...
        db_execute_prepared(
            'UPDATE mac_track_interface_graphs
			SET present = 0
			WHERE device_id = ?
			AND host_id = ?',
            [$device_id, $host_id]
        ...
        db_execute_prepared(
            'DELETE FROM mac_track_interface_graphs
			WHERE present = 0
			AND device_id = ?
			AND host_id = ?',
            [$device_id, $host_id]
...
function get_base_dot1dTpFdbEntry_ports($site, &$device, &$ifInterfaces, $snmp_readstring = '', $store_to_db = true, $lowPort = 1, $highPort = 9999)
{
    ...
    if (cacti_sizeof($active_ports_array)) {
        foreach ($active_ports_array as $port_info) {
            ...
            if (isset($indexes[$i], $ifInterfaces[$indexes[$i]]['ifType'])) {
                if ((($ifInterfaces[$indexes[$i]]['ifType'] >= 6)
                     && ($ifInterfaces[$indexes[$i]]['ifType'] <= 9))
                    || (53 == $ifInterfaces[$indexes[$i]]['ifType']) // vlan
                    || (161 == $ifInterfaces[$indexes[$i]]['ifType']) // port-channel
                    || (71 == $ifInterfaces[$indexes[$i]]['ifType'])) {
                    if (1 == $port_info) {
                        ++$ports_active;
                    }
                }
            ...
            foreach ($port_numbers as $key => $port_number) {
                if ((0 == $highPort)
                    ...
                    if (!in_array($port_number, $ignore_ports)) {
                        if ((isset($port_status[$key]) && '3' == $port_status[$key])
                            || (isset($port_status[$key]) && '5' == $port_status[$key])) {
            ...
            foreach ($port_key_array as $port_key) {
                ...
                if ($port_key['port_number'] > 0) {
                    ...
                    if ((($brPortIfType >= 6 && $brPortIfType <= 9)
                          || 53 == $brPortIfType || 161 == $brPortIfType)
...
function get_ios_vrf_arp_table($oid, &$device, $snmp_readstring = '', $hex = false)
{
    ...
    if ('3' == $device['snmp_version'] && substr_count($snmp_readstring, 'vlan-')) {
        $snmp_context = $snmp_readstring;
    } else {
        $snmp_context = $device['snmp_context'];
    }

    $walk_array = cacti_snmp_walk(
        $device['hostname'],
        $snmp_readstring,
        $oid,
        $device['snmp_version'],
...
function get_base_wireless_dot1dTpFdbEntry_ports($site, &$device, &$ifInterfaces, $snmp_readstring = '', $store_to_db = true, $lowPort = 1, $highPort = 9999)
{
    ...
    if ($ports_active > 0) {
        ...
        $bridge_root_port = @cacti_snmp_get(
            $device['hostname'],
            $snmp_readstring,
        ...
        if (cacti_sizeof($port_numbers)) {
            foreach ($port_numbers as $key => $port_number) {
                if ((0 == $highPort)
                    ...
                    if (!in_array($port_number, $ignore_ports)) {
                        if (('3' == @$port_status[$key]) || ('5' == @$port_status[$key])) {
                            $port_key_array[$i]['key'] = $key;
                            $port_key_array[$i]['port_number'] = $port_number;
        ...
        if (cacti_sizeof($port_key_array)) {
            foreach ($port_key_array as $port_key) {
                ...
                if ($port_key['port_number'] > 0) {
                    ...
                    if ((($brPortIfType >= 6) && ($brPortIfType <= 9)) || (71 == $brPortIfType)) {
                        // set some defaults
                        $new_port_key_array[$i]['vlan_id'] = 'N/A';
                        $new_port_key_array[$i]['vlan_name'] = 'N/A';
                        $new_port_key_array[$i]['mac_address'] = 'NOT USER';
        ...
        if (cacti_sizeof($port_numbers)) {
            foreach ($port_numbers as $key => $port_number) {
                if ((0 == $highPort)
                    ...
                    && ($bridge_root_port != $port_number))) {
                    if (!in_array($port_number, $ignore_ports)) {
                        if ((isset($port_status[$key]) && '3' == $port_status[$key]) || (isset($port_status[$key]) && '5' == $port_status[$key])) {
                            $port_key_array[$i]['key'] = $key;
                            $port_key_array[$i]['port_number'] = $port_number;
...
function db_process_add($device_id, $storepid = false)
{
    ...
    db_execute_prepared(
        'REPLACE INTO mac_track_processes
...
{
    db_execute_prepared(
        'DELETE FROM mac_track_processes
		WHERE device_id = ?',
        [$device_id]
    );
}
...
function db_store_device_port_results(&$device, $port_array, $scan_date)
{
    ...
    if (cacti_sizeof($port_array)) {
        foreach ($port_array as $port_value) {
            if ('NOT USER' != $port_value['port_number'] && 'NOT USER' != $port_value['mac_address'] && '' != $port_value['mac_address']) {
                ...
                db_execute_prepared(
                    'REPLACE INTO mac_track_temp_ports
...
function perform_mactrack_db_maint()
{
    ...
    if ($retention > 0 || $partitioned) {
        if (!$partitioned) {
            ...
            $syslog_deleted = 0;
            $number_of_partitions = db_fetch_assoc_prepared(
                'SELECT *
                ...
                if ('' != $lday_ts) {
                    ...
                    if ($days > 0) {
                        ...
                        */
                        ...
                        $old_partitions = db_fetch_assoc_prepared(
...
function import_oui_database($type = 'ui', $oui_file = 'http://standards-oui.ieee.org/oui.txt')
{
    ...
    if ('ui' != $type) {
        ...
        echo __('Getting OUI Database from the IEEE', 'mactrack').PHP_EOL;
    }
    ...
    if (is_array($oui_database)) {
        echo __('OUI Database Download from IEEE Complete', 'mactrack').PHP_EOL;
    } else {
        echo __('OUI Database Download from IEEE FAILED', 'mactrack').PHP_EOL;
    }
    ...
    if (is_array($oui_database)) {
        ...
        echo PHP_EOL.__('There were \'%d\' Entries Added/Updated in the database.', $i, 'mactrack');
        if ('ui' != $type) {
            echo '</td></td><tr><td>';
        }
        echo PHP_EOL.__('There were \'%d\' Records Removed from the database.', $j, 'mactrack').PHP_EOL;
        if ('ui' != $type) {
            echo '</td></tr>';
        }
...
function mactrack_interface_actions($device_id, $ifIndex, $show_rescan = true)
{
    ...
    if (0 != $device['host_id']) {
        ...
        $graphs = db_fetch_assoc_prepared(
            'SELECT DISTINCT gl.id AS local_graph_id
...
function mactrack_format_interface_row($stat)
{
    ...
    if ('true' == get_request_var('totals') || 'on' == get_request_var('totals')) {
    ...
    form_selectable_cell(1 == $stat['ifOperStatus'] ? __('Up', 'mactrack') : __('Down', 'mactrack'), $stat['device_id'], '', 'right');
...
function mactrack_format_dot1x_row($port_result)
{
    ...
    $row .= '<td><b>'.(2 == $port_result['domain'] ? __('Data', 'mactrack') : __('Voice', 'mactrack')).'</b></td>';
...
{
    if ('1' == $stat['int_errors_present']) {
        return 'int_errors';
    }
    if ('1' == $stat['int_discards_present']) {
        return 'int_discards';
    }
    if ('1' == $stat['ifOperStatus'] && '' == $stat['ifAlias']) {
        return 'int_up_wo_alias';
    }
    if ('0' == $stat['ifOperStatus']) {
        return 'int_down';
    }
...
{
    if ('7' == $port_result['status']) {
        return 'dot1x_authn_failed';
    }
    if ('5' == $port_result['status']) {
        return 'dot1x_auth_failed';
    }
    if ('3' == $port_result['status']) {
        return 'dot1x_auth_no_method';
    }
    if ('2' == $port_result['status']) {
        return 'dot1x_running';
    }
    if ('1' == $port_result['status']) {
        return 'dot1x_idle';
    }
    if ('4' == $port_result['status']) {
...
function mactrack_create_sql_filter($filter, $fields)
{
    ...
    foreach ($elements as $element) {
        ...
        foreach ($fields as $field) {
            ...
            $query .= (1 == $field_no ? '' : " {$operator} ")."({$field} {$type} LIKE '%".$filter."%')";
...
function mactrack_display_stats()
{
    ...
    if ($processes > 0) {
        ...
        $message = __(
            ...
            $proc,
            $devs,
            'disabled' != $timing ? date('Y-m-d H:i:s', strtotime(read_config_option('mt_scan_date', true)) + $frequency) : __('Disabled', 'mactrack'),
            'mactrack'
        );
    }
    ...
    echo '<td>'.__('Scanning Rate: Every %s', mactrack_display_hours(read_config_option('mt_collection_timing')), 'mactrack').', '.$message.'</td>';
    echo '</tr>';
...
{
    echo "<td width='16.67%' class='{$class}' style='text-align:center;;'>{$text}</td>";
}
...
function mactrack_format_device_row($device, $actions = false)
{
    ...
    if ($actions) {
        ...
        echo "<td style='width:40px;'>".$row.'</td>';
    }
    ...
    form_selectable_cell(get_colored_device_status('on' == $device['disabled'] ? true : false, $device['snmp_status']), $device['device_id']);
    ...
    form_selectable_cell('1' == $device['scan_type'] ? __('N/A', 'mactrack') : number_format_i18n($device['ips_total'], -1), $device['device_id'], '', 'right');
    form_selectable_cell('3' == $device['scan_type'] ? __('N/A', 'mactrack') : number_format_i18n($device['ports_total'], -1), $device['device_id'], '', 'right');
    form_selectable_cell('3' == $device['scan_type'] ? __('N/A', 'mactrack') : number_format_i18n($device['ports_active'], -1), $device['device_id'], '', 'right');
    form_selectable_cell('3' == $device['scan_type'] ? __('N/A', 'mactrack') : number_format_i18n($device['ports_trunk'], -1), $device['device_id'], '', 'right');
    form_selectable_cell('3' == $device['scan_type'] ? __('N/A', 'mactrack') : number_format_i18n($device['macs_active'], -1), $device['device_id'], '', 'right');
...
function mactrack_tabs()
{
    ...
    // draw the tabs
    echo "<div class='tabs'><nav><ul>\n";

    if (cacti_sizeof($tabs_mactrack)) {
        foreach ($tabs_mactrack as $tab_short_name => $tab_name) {
            echo '<li><a class="tab pic'.(($tab_short_name == $current_tab) ? ' selected"' : '"')." href='".htmlspecialchars($config['url_path']
    ...
    echo "</ul></nav><script type='text/javascript'>\n";

    echo "$(function() { if (pageName.indexOf('mactrack_view') >= 0) { $('.maintabs a.selected').attr('href', urlPath+'plugins/mactrack/'+pageName); } });";
    echo '</script></div>';
}
...
function mactrack_site_filter($page = 'mactrack_sites.php')
{
    ...
    ?>
					...
					<td>
						<input type='checkbox' id='detail' <?php if ('true' == get_request_var('detail')) {
						    echo ' checked="true"';
						}?> onClick='applyFilter()'>
					</td>
					<td>
						<label for='detail'><?php echo __('Show Device Details', 'mactrack'); ?></label>
					</td>
					<td>
						<span>
							<button type='submit' id='go' class='ui-button ui-corner-all ui-widget ui-state-active'><?php echo __('Go', 'mactrack'); ?></button>
							<button type='button' id='clear' class='ui-button ui-corner-all ui-widget'><?php echo __('Clear', 'mactrack'); ?></button>
							<button type='button' id='export' class='ui-button ui-corner-all ui-widget'><?php echo __('Export', 'mactrack'); ?></button>
						</span>
					</td>
				</tr>
			<?php
            if (!('false' == get_request_var('detail'))) { ?>
			</table>
			<table class='filterTable'>
				<tr>
					<td>
					...
					<td>
						<select id='site_id' onChange='applyFilter()'>
							<option value='-1'<?php if ('-1' == get_request_var('site_id')) {?> selected<?php }?>><?php echo __('Any', 'mactrack'); ?></option>
							<?php
                ...
                if (cacti_sizeof($device_types)) {
                    foreach ($device_types as $device_type) {
                        ...
                        if (get_request_var('device_type_id') == $device_type['device_type_id']) {
                            echo ' selected';
                        } echo '>'.$device_type['description'].' ('.$device_type['sysDescr_match'].')</option>';
                    }
                }
                ?>
						</select>
			...
			function applyFilter() {
				strURL  = urlPath+'plugins/mactrack/<?php echo $page; ?>?header=false';
				strURL += '&report=sites';
			...
			function clearFilter() {
				strURL  = urlPath+'plugins/mactrack/<?php echo $page; ?>?header=false&clear=true';
				loadPageNoHeader(strURL);
			}

			function exportRows() {
				strURL  = urlPath+'plugins/mactrack/<?php echo $page; ?>?export=true';
...
if (!function_exists('cacti_sizeof')) {
    ...
    {
        return (false === $array || !is_array($array)) ? 0 : sizeof($array);
    }
}
...
if (!function_exists('cacti_count')) {
    ...
    {
        return (false === $array || !is_array($array)) ? 0 : count($array);
    }
}
...
function mactrack_format_mac($mac)
{
    ...
    if ('aabbccddeeff' == $format) {
        return $mac;
    }

    if ('aabb-ccdd-eeff' == $format) {
