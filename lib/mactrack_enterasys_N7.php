...
function xform_enterasys_N7_vlan_associations(&$device, $snmp_readstring = '')
{
    ...
    foreach ($xformArray as $xformItem) {
        ...
        $vlan_name = @cacti_snmp_get(
            $device['hostname'],
            $snmp_readstring,
            '.1.3.6.1.2.1.17.7.1.4.3.1.1.'.$output_array[$i]['vlan_id'],
            $device['snmp_version'],
            '',
            '',
            '',
...
function get_CTAlias_table($site, &$device)
{
    ...
    if (cacti_sizeof($CTAliasEntries)) {
        ...
        foreach ($CTAliasEntries as $CTAliasEntry) {
            ...
            if (1 != $CTAliasEntry['CTAliasProtocol']) {
                continue;
            }
            ...
            $sql[] = '('
                ...
                .db_qstr($CTAliasEntry['ifIndex']).', '
                .db_qstr($CTAliasEntry['CTAliasMacAddress']).', '
                .db_qstr($CTAliasEntry['CTAliasAddressText']).', '
                .db_qstr($scan_date).')';
        }
