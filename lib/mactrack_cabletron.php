...
function get_cabletron_switch_ports($site, &$device, $lowPort, $highPort)
{
    ...
    $securefast_marker = @cacti_snmp_get(
        $device['hostname'],
        $device['snmp_readstring'],
        '.1.3.6.1.4.1.52.4.2.4.2.1.1.1.1.1.1.1',
        $device['snmp_version'],
...
function get_repeater_snmp_readstring(&$device)
{
    $active_ports = @cacti_snmp_get(
        $device['hostname'],
        $device['snmp_readstring'],
        '.1.3.6.1.4.1.52.4.1.1.1.4.1.1.4.0',
        $device['snmp_version'],
        $device['snmp_username'],
        $device['snmp_password'],
        $device['snmp_auth_protocol'],
...
function get_repeater_rev4_ports($site, &$device, $lowPort, $highPort)
{
    ...
    if ('' != $snmp_readstring) {
        $ports_active = @cacti_snmp_get(
            $device['hostname'],
            $snmp_readstring,
            '.1.3.6.1.4.1.52.4.1.1.1.4.1.1.5.0',
            $device['snmp_version'],
            $device['snmp_username'],
            $device['snmp_password'],
            $device['snmp_auth_protocol'],
