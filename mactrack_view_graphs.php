...
function mactrack_view_graphs() {
	...
	if (get_filter_request_var('host_id') > 0) {
		$sql_where .= ($sql_where != '' ? ' AND' : '') . ' gl.host_id=' . get_filter_request_var('host_id');
	}

	// Graph Template Id sql_where
	if (get_filter_request_var('graph_template_id') > 0) {
		$sql_where .= ($sql_where != '' ? ' AND' : '') . ' gl.graph_template_id=' . get_filter_request_var('graph_template_id');
	}
