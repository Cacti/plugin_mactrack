...
function form_actions() {
	...
	html_start_box("<strong>" . $maca_actions{$_POST["drp_action"]} . "</strong>", "60%", "", "3", "center", "");
	...
	if (!isset($maca_array)) {
		print "<tr><td class='even'><span class='textError'>You must select at least one Authorized Mac to delete.</span></td></tr>\n";
		$save_html = "";
	}else{
...
function mactrack_maca_remove() {
	...
	if ((read_config_option("remove_verification") == "on") && (!isset($_REQUEST["confirm"]))) {
		...
		form_confirm("Are You Sure?", "Are you sure you want to delete the Authorized Mac Address(s)<strong>'" . db_fetch_cell("SELECT mac_address FROM mac_track_macauth WHERE mac_id" . $_REQUEST["mac_id"]) . "'</strong>?", "mactrack_macauth.php", "mactrack_macauth.php?action=remove&mac_id=" . $_REQUEST["mac_id"]);
		include("./include/bottom_footer.php");
		exit;
	}

	if ((read_config_option("remove_verification") == "") || (isset($_REQUEST["confirm"]))) {
		api_mactrack_maca_remove($_REQUEST["mac_id"]);
	}
}
...
function mactrack_maca_edit() {
	...
	if (!empty($_REQUEST["mac_id"])) {
		$mac_record = db_fetch_row("SELECT * FROM mac_track_macauth WHERE mac_id=" . $_REQUEST["mac_id"]);
		$header_label = "[edit: " . $mac_record["mac_address"] . "]";
	}else{
		$header_label = "[new]";
	}

	html_start_box("<strong>MacTrack MacAuth</strong> $header_label", "100%", "", "3", "center", "");
...
function mactrack_maca() {
	...
	html_start_box("<strong>MacTrack MacAuth Filters</strong>", "100%", "", "3", "center", "mactrack_macauth.php?action=edit");
	...
	$nav = html_nav_bar("mactrack_macauth.php?filter=" . $_REQUEST["filter"], MAX_DISPLAY_PAGES, get_request_var_request("page"), $row_limit, $total_rows, 9, 'Authorized Mac Addresses');
	...
	if (sizeof($maca) > 0) {
		foreach ($maca as $mac) {
			form_alternate_row();
				?>
				<td width="20%">
					<a class="linkEditMain" href="mactrack_macauth.php?action=edit&mac_id=<?php print $mac['mac_id'];?>"><?php print (strlen($_REQUEST["filter"]) ? preg_replace("/(" . preg_quote($_REQUEST["filter"]) . ")/i", "<span class='filteredValue'>\\1</span>", $mac["mac_address"]) : $mac["mac_address"]);?></a>
				</td>
				<td width="50%"><?php print (strlen($_REQUEST["filter"]) ? preg_replace("/(" . preg_quote($_REQUEST["filter"]) . ")/i", "<span class='filteredValue'>\\1</span>", $mac["description"]) : $mac["description"]);?></td>
