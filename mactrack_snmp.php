...
function form_mactrack_snmp_actions()
{
    ...
    if (isset_request_var('selected_items')) {
        ...
        if (false != $selected_items) {
            if ('1' == get_nfilter_request_var('drp_action')) { // delete
    ...
    foreach ($_POST as $var => $val) {
        if (preg_match('/^chk_([0-9]+)$/', $var, $matches)) {
            ...
            $snmp_groups .= '<li>'.db_fetch_cell_prepared('SELECT name FROM mac_track_snmp WHERE id = ?', [$matches[1]]).'</li>';
    ...
    if (!isset($mactrack_array)) {
        echo "<tr><td class='even'><span class='textError'>".__('You must select at least one SNMP Option.', 'mactrack')."</span></td></tr>\n";
        $save_html = '';
    } else {
        ...
        if ('1' == get_nfilter_request_var('drp_action')) { // delete
            ...
            echo "<tr>
				<td class='textArea'>
					...
					<ul>{$snmp_groups}</ul>
					<p>".__('Name Format:', 'mactrack').'<br>';
            form_text_box('name_format', __('<name> (1)', 'mactrack'), '', '255', '30', 'text');
            echo '</p>
				</td>
			</tr>';
        }
...
function mactrack_snmp_item_remove()
{
    ...
    db_execute_prepared(
        'DELETE FROM mac_track_snmp_items
		WHERE id = ?',
...
function mactrack_snmp_item_edit()
{
    ...
    $snmp_option = db_fetch_row_prepared(
        'SELECT *
		FROM mac_track_snmp
		WHERE id = ?',
        [get_request_var('id')]
    ...
    if ('' !== get_request_var('item_id', '')) {
        $mactrack_snmp_item = db_fetch_row_prepared(
            'SELECT *
...
function mactrack_snmp_edit()
{
    ...
    if (!isempty_request_var('id')) {
        $snmp_group = db_fetch_row_prepared(
            'SELECT *
			FROM mac_track_snmp
			WHERE id = ?',
            [get_request_var('id')]
    ...
    if (!isempty_request_var('id')) {
        ...
        if (cacti_sizeof($items)) {
            foreach ($items as $item) {
                ...
                $form_data .= '<td>'.(3 == $item['snmp_version'] ? __('N/A', 'mactrack') : $item['snmp_readstring']).'</td>';
                ...
                $form_data .= '<td>'.(3 == $item['snmp_version'] ? $item['snmp_username'] : __('N/A', 'mactrack')).'</td>';
                $form_data .= '<td>'.(3 == $item['snmp_version'] ? $item['snmp_auth_protocol'] : __('N/A', 'mactrack')).'</td>';
                $form_data .= '<td>'.(3 == $item['snmp_version'] ? $item['snmp_priv_protocol'] : __('N/A', 'mactrack')).'</td>';
            ...
            echo '<tr><td colspan="5"><em>'.__('No SNMP Items Found', 'mactrack').'</em></td></tr>';
        }
...
function mactrack_snmp()
{
    ...
    $display_text = [
        'name' => [__('Title of SNMP Option Set', 'mactrack'), 'ASC'],
    ];
    ...
    if (cacti_sizeof($snmp_groups)) {
        ...
        echo '<tr><td colspan="3"><em>'.__('No SNMP Option Sets Found', 'mactrack').'</em></td></tr>';
    }
		...
		<form id='mactrack_snmp' action='mactrack_snmp.php'>
			<table class='filterTable'>
				<tr>
					...
					<td>
						<?php echo __('Options', 'mactrack'); ?>
					</td>
					<td>
						<select id='rows' onChange='applyFilter()'>
							<option value='-1'<?php echo '-1' == get_request_var('rows') ? ' selected' : ''; ?>><?php echo __('Default', 'mactrack'); ?></option>
					...
					<td>
						<span>
							<button type='button' id='go' class='ui-button ui-corner-all ui-widget ui-state-active'><?php echo __esc('Go', 'mactrack'); ?></button>
							<button type='button' id='clear' class='ui-button ui-corner-all ui-widget'><?php echo __esc('Clear', 'mactrack'); ?></button>
						</span>
					</td>
				</tr>
			</table>
			<input type='hidden' name='page' value='<?php echo get_request_var('page'); ?>'>
		</td>
		</form>
		<script type='text/javascript'>
