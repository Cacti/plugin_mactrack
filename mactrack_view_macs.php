...
function mactrack_view_get_mac_records(&$sql_where, &$sql_params, $rows, $apply_limits = true) {
	...
	if (get_request_var('authorized') != '-1') {
		$sql_where .= ($sql_where != '' ? ' AND' : 'WHERE') . ' mtp.authorized = ' . get_filter_request_var('authorized');
	}

	if (get_request_var('site_id') != '-1') {
		$sql_where .= ($sql_where != '' ? ' AND' : 'WHERE') . ' mtp.site_id = ' . get_filter_request_var('site_id');
	}

	if (get_request_var('vlan') != '-1') {
		$sql_where .= ($sql_where != '' ? ' AND' : 'WHERE') . ' mtp.vlan_id = ' . get_filter_request_var('vlan');
	}

	if (get_request_var('device_id') != '-1') {
		$sql_where .= ($sql_where != '' ? ' AND' : 'WHERE') . ' mtp.device_id = ' . get_filter_request_var('device_id');
	}
...
function mactrack_view_macs() {
	...
	$port_results = mactrack_view_get_mac_records($sql_where, $sql_params, $rows, true);
...
function mactrack_mac_filter() {
	...
	if (get_request_var('site_id') != '-1') {
		if ($sql_where != '') {
			$sql_where .= ' AND site_id=' . get_filter_request_var('site_id');
		} else {
			$sql_where = 'WHERE site_id=' . get_filter_request_var('site_id');
		}
	}
