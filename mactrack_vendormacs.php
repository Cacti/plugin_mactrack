...
function mactrack_vmacs()
{
    ...
    if (cacti_sizeof($vmacs)) {
        foreach ($vmacs as $vmac) {
            form_alternate_row();
            ?>
				<td class='linkEditMain'><?php echo $vmac['vendor_mac']; ?></td>
				<td><?php echo filter_value($vmac['vendor_name'], get_request_var('filter')); ?></td>
				<td><?php echo filter_value(mactrack_format_mac($vmac['vendor_address']), get_request_var('filter')); ?></td>
			</tr>
			<?php
        }
    } else {
        echo '<tr><td colspen="'.$columns.'"><em>'.__('No Mactrack Vendor MACS Found', 'mactrack').'</em></td></tr>';
    }
