...
function mactrack_view_get_ip_records(&$sql_where, $apply_limits = true, $rows)
{
    ...
    } elseif (('1' != get_request_var('scan_date')) && '' != get_request_var('scan_date')) {
...
function mactrack_view_ips()
{
    ...
    if ('' == $sql_where) {
        $total_rows = 0;
    } elseif (1 == get_request_var('rows') || (1 == get_request_var('scan_date'))) {
    ...
    if (cacti_sizeof($port_results)) {
        ...
        if (-1 == get_request_var('site_id') && -1 == get_request_var('device_id')) {
            ...
            echo '<tr><td colspan="10"><em>'.__('No Mactrack IP Results Found', 'mactrack').'</em></td></tr>';
        }
    }
...
function mactrack_ip_address_filter()
{
    ...
    if (-1 == get_request_var('site_id')) {
        ...
        $filter_devices = db_fetch_assoc_prepared(
            'SELECT DISTINCT device_id, device_name, hostname
									FROM mac_track_devices
									WHERE site_id = ?
									ORDER BY device_name',
