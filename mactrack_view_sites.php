...
function mactrack_view_get_site_records(&$sql_where, $rows, $apply_limits = true)
{
    ...
    $device_type_info = db_fetch_row_prepared(
        'SELECT *
    ...
    if (('-1' != get_request_var('site_id')) && get_request_var('detail')) {
...
function mactrack_view_sites()
{
    ...
    if ('false' == get_request_var('detail')) {
        ...
        if (cacti_sizeof($sites)) {
            ...
            echo '<tr><td colspan="'.$columns.'"><em>'.__('No Mactrack Sites Found', 'mactrack').'</em></td></tr>';
        }
        ...
        if (cacti_sizeof($sites)) {
            foreach ($sites as $site) {
                ...
                form_selectable_cell(number_format_i18n($site['total_devices']), 'row_'.$site['site_id'], '', 'right');
                form_selectable_cell('1' == $dt ? __('N/A', 'mactrack') : number_format_i18n($site['sum_ips_total']), 'row_'.$site['site_id'], '', 'right');
                form_selectable_cell('3' == $dt ? __('N/A', 'mactrack') : number_format_i18n($site['sum_ports_total']), 'row_'.$site['site_id'], '', 'right');
                form_selectable_cell('3' == $dt ? __('N/A', 'mactrack') : number_format_i18n($site['sum_ports_active']), 'row_'.$site['site_id'], '', 'right');
                form_selectable_cell('3' == $dt ? __('N/A', 'mactrack') : number_format_i18n($site['sum_ports_trunk']), 'row_'.$site['site_id'], '', 'right');
                form_selectable_cell('3' == $dt ? __('N/A', 'mactrack') : number_format_i18n($site['sum_macs_active']), 'row_'.$site['site_id'], '', 'right');
                form_end_row();
            }
        } else {
            echo '<tr><td colspan="'.$columns.'"><em>'.__('No Mactrack Sites Found', 'mactrack').'</em></td></tr>';
    ...
    echo '<div id="response" title="'.__esc('Site Scan Results', 'mactrack').'"></div>';
}
