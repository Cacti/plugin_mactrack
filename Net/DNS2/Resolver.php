...
class Net_DNS2_Resolver extends Net_DNS2
{
    ...
    public function query($name, $type = 'A', $class = 'IN')
    {
        ...
        //
        if (true == $this->dnssec_ad_flag) {
            $packet->header->ad = 1;
        }
        if (true == $this->dnssec_cd_flag) {
            $packet->header->cd = 1;
        }
        ...
        if ((true == $this->use_cache) && (true == $this->cacheable($type))) {
            //
            // open the cache
            //
            $this->cache->open(
        ...
        //
        if ((true == $this->strict_query_mode)
            ...
            foreach ($response->answer as $index => $object) {
                if ((0 == strcasecmp(trim($object->name, '.'), trim($packet->question[0]->qname, '.')))
