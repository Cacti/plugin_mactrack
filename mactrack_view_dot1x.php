...
{
    // status sql where
    if ('1' == get_request_var('status')) { // Idle
        $sql_where .= ('' != $sql_where ? ' AND ' : 'WHERE ').'mtd.status = 1';
    } elseif ('2' == get_request_var('status')) { // Running
        $sql_where .= ('' != $sql_where ? ' AND ' : 'WHERE ').'mtd.status = 2';
    } elseif ('3' == get_request_var('status')) { // No Method
        $sql_where .= ('' != $sql_where ? ' AND ' : 'WHERE ').'mtd.status = 3';
    } elseif ('4' == get_request_var('status')) { // Authentication Success
        $sql_where .= ('' != $sql_where ? ' AND ' : 'WHERE ').'mtd.status = 4';
    } elseif ('5' == get_request_var('status')) { // Authentication Failed
        $sql_where .= ('' != $sql_where ? ' AND ' : 'WHERE ').'mtd.status = 5';
    } elseif ('6' == get_request_var('status')) { // Authorization Success
        $sql_where .= ('' != $sql_where ? ' AND ' : 'WHERE ').'mtd.status = 6';
    } elseif ('7' == get_request_var('status')) { // Authorization Failed
    ...
    if (('1' != get_request_var('scan_date')) && ('2' != get_request_var('scan_date'))) {
...
function mactrack_view_dot1x()
{
    ...
    if ('' == $sql_where) {
        $total_rows = 0;
    } elseif (3 != get_request_var('scan_date')) {
    ...
    if (cacti_sizeof($port_results)) {
        foreach ($port_results as $port_result) {
            ...
            if ($class) {
                echo "<tr id='row_".$port_result['device_id'].'_'.$port_result['port_number']."' class='tableRow {$class}'>\n";
                ++$i;
            } else {
                if (($i % 2) == 1) {
                    $class = 'odd';
                ...
                echo "<tr id='row_".$port_result['device_id']."' class='tableRow {$class}'>\n";
                ++$i;
            }

            echo mactrack_format_dot1x_row($port_result);
        }
    } else {
        echo '<tr><td colspan="7"><em>'.__('No Mactrack 802.1x Sessions Found', 'mactrack').'</em></td></tr>';
    }
    ...
    echo '<div id="response" title="'.__esc('Dot1x Scan Results', 'mactrack').'"></div>';
...
function mactrack_dot1x_filter()
{
		...
		<td>
			...
			<table class='filterTable'>
				<tr>
					...
					<td>
						<input type='text' id='filter' size='25' value='<?php echo html_escape_request_var('filter'); ?>'>
					</td>
					<td>
    ...
    if (cacti_sizeof($filter_devices)) {
        foreach ($filter_devices as $filter_device) {
            echo '<option value="'.$filter_device['device_id'].'"'.(get_request_var('device_id') == $filter_device['device_id'] ? ' selected' : '').'>'.html_escape($filter_device['device_name'].'('.$filter_device['hostname'].')').'</option>';
        }
    }
    ?>
						</select>
    ...
    if (cacti_sizeof($rows_selector)) {
        foreach ($rows_selector as $key => $value) {
            echo '<option value="'.$key.'"'.(get_request_var('rows') == $key ? ' selected' : '').'>'.$value.'</option>';
        }
    }
    ?>
						</select>
    ...
    foreach ($mactrack_search_types as $i => $type) {
        echo "<option value='{$i}'".(get_request_var('ip_filter_type_id') == $i ? ' selected' : '').'>'.$type.'</option>';
    }
    ?>
						</select>
					</td>
    ...
    foreach ($all_status as $i => $status) {
        echo "<option value='{$i}'".(get_request_var('status') == $i ? ' selected' : '').'>'.$status.'</option>';
    }
    ?>
						</select>
					</td>
    ...
    if (cacti_sizeof($scan_dates)) {
        foreach ($scan_dates as $scan_date) {
            echo '<option value="'.$scan_date['scan_date'].'"'.(get_request_var('scan_date') == $scan_date['scan_date'] ? ' selected' : '').'>'.$scan_date['scan_date'].'</option>';
        }
    }
    ?>
						</select>
				...
				<tr>
					...
					<td>
						<select id='mac_filter_type_id'>
							<?php
    foreach ($mactrack_search_types as $i => $type) {
        echo "<option value='{$i}'".(get_request_var('mac_filter_type_id') == $i ? ' selected' : '').'>'.$type.'</option>';
    }
    ?>
						</select>
					</td>
					...
					<td>
						<?php echo __('Domain', 'mactrack'); ?>
					</td>
					<td>
						<select id='domain' onChange='applyFilter()'>
							<option value='-1'<?php if ('-1' == get_request_var('domain')) {?> selected<?php }?>><?php echo __('All', 'mactrack'); ?></option>
							<option value='2'<?php if ('2' == get_request_var('domain')) {?> selected<?php }?>><?php echo __('DATA', 'mactrack'); ?></option>
							<option value='3'<?php if ('3' == get_request_var('domain')) {?> selected<?php }?>><?php echo __('VOICE', 'mactrack'); ?></option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<select id='port_name_filter_type_id'>
							<?php
    foreach ($mactrack_search_types as $i => $type) {
        echo "<option value='{$i}'".(get_request_var('port_name_filter_type_id') == $i ? ' selected' : '').'>'.$type.'</option>';
    }
    ?>
						</select>
					</td>
