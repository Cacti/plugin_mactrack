...
function form_save()
{
    if (isset_request_var('save_component_device_type') && isempty_request_var('add_dq_y')) {
        $device_type_id = api_mactrack_device_type_save(
            get_nfilter_request_var('device_type_id'),
            get_nfilter_request_var('description'),
            get_nfilter_request_var('vendor'),
            get_nfilter_request_var('device_type'),
            get_nfilter_request_var('sysDescr_match'),
            get_nfilter_request_var('sysObjectID_match'),
            get_nfilter_request_var('scanning_function'),
            get_nfilter_request_var('ip_scanning_function'),
            get_nfilter_request_var('dot1x_scanning_function'),
            get_nfilter_request_var('serial_number_oid'),
            get_nfilter_request_var('lowPort'),
...
{
    db_execute_prepared(
        'DELETE FROM mac_track_device_types
		WHERE device_type_id = ?',
        [$device_type_id]
    );
}

function api_mactrack_device_type_save(
    $device_type_id,
    $description,
    $vendor,
    $device_type,
...
function api_mactrack_duplicate_device_type($device_type_id, $dup_id, $device_type_title)
{
    if (!empty($device_type_id)) {
        $device_type = db_fetch_row_prepared(
            'SELECT *
...
function form_actions()
{
    ...
    if (isset_request_var('selected_items')) {
        ...
        if (false != $selected_items) {
            if ('1' == get_request_var('drp_action')) { // delete
                for ($i = 0; $i < cacti_sizeof($selected_items); ++$i) {
                    api_mactrack_device_type_remove($selected_items[$i]);
                }
            } elseif ('2' == get_nfilter_request_var('drp_action')) { // duplicate
                for ($i = 0; $i < cacti_sizeof($selected_items); ++$i) {
    ...
    foreach ($_POST as $var => $val) {
        if (preg_match('/^chk_([0-9]+)$/', $var, $matches)) {
            ...
            $device_types_info = db_fetch_row_prepared(
                'SELECT description
				FROM mac_track_device_types
				WHERE device_type_id = ?',
                [$matches[1]]
    ...
    if (!isset($device_types_array)) {
        echo "<tr><td class='even'><span class='textError'>".__('You must select at least one device type.', 'mactrack')."</span></td></tr>\n";
        $save_html = '';
    } else {
        ...
        } elseif ('2' == get_request_var('drp_action')) { // duplicate
            echo "<tr>
				<td class='textArea'>
...
function mactrack_rescan_device_types()
{
    ...
    if (cacti_sizeof($insert_array)) {
        ...
        echo __('There were %d Device Types Added!', cacti_sizeof($insert_array), 'mactrack');
    } else {
        echo __('No New Device Types Found!', 'mactrack');
    }
}
...
function mactrack_device_type_import()
{
    ...
    form_alternate_row(); ?>
		<td width='50%'><font class='textEditTitle'><?php echo __('Import Device Types from Local File', 'mactrack'); ?></font><br>
			<?php echo __('Please specify the location of the CSV file containing your device type information.', 'mactrack'); ?>
		</td>
		<td align='left'>
    ...
    form_alternate_row(); ?>
		<td><strong><?php echo __('The file must contain a header row with the following column headings.', 'mactrack'); ?></strong>
			<br><br>
			<strong>description</strong><?php echo __(' - A common name for the device.  For example Cisco 6509 Switch', 'mactrack'); ?><br>
			<strong>vendor</strong><?php echo __(' - The vendor who produces this device', 'mactrack'); ?><br>
			<strong>device_type</strong><?php echo __(' - The type of device this is.  See the notes below for this integer value', 'mactrack'); ?><br>
			<strong>sysDescr_match</strong><?php echo __(' - A unique set of characters from the snmp sysDescr that uniquely identify this device', 'mactrack'); ?><br>
			<strong>sysObjectID_match</strong><?php echo __(' - The vendor specific snmp sysObjectID that distinguishes this device from the next', 'mactrack'); ?><br>
			<strong>scanning_function</strong><?php echo __(' - The scanning function that will be used to scan this device type', 'mactrack'); ?><br>
			<strong>ip_scanning_function</strong><?php echo __(' - The IP scanning function that will be used to scan this device type', 'mactrack'); ?><br>
			<strong>dot1x_scanning_function</strong><?php echo __(' - The 802.1x scanning function that will be used to scan this device type', 'mactrack'); ?><br>
			<strong>serial_number_oid</strong><?php echo __(' - If the Serial Number for this device type can be obtained via an SNMP Query, add it\'s OID here', 'mactrack'); ?><br>
			<strong>lowPort</strong><?php echo __(' - If your scanning function does not have the ability to isolate trunk ports or link ports, this is the starting port number for user ports', 'mactrack'); ?><br>
			<strong>highPort</strong><?php echo __(' - Same as the lowPort with the exception that this is the high numbered user port number', 'mactrack'); ?><br>
			<strong>disabled</strong><?php echo __(' - Disabled type is not used', 'mactrack'); ?><br>
			<br>
			...
			<br><br>
			<strong><?php echo __('Therefore, if you attempt to import duplicate device types, the existing data will be updated with the new information.', 'mactrack'); ?></strong>
			<br><br>
			<strong>device_type</strong><?php echo __(' is an integer field and must be one of the following:', 'mactrack'); ?>
			<br><br>
			<?php echo __('1 - Switch/Hub', 'mactrack'); ?><br>
			<?php echo __('2 - Switch/Router', 'mactrack'); ?><br>
			<?php echo __('3 - Router', 'mactrack'); ?><br>
			<br>
			<strong><?php echo __('The devices device type is determined by scanning it\'s snmp agent for the sysObjectID and sysDescription and comparing it against values in the device types database.  The first match that is found in the database is used as to how to scan it.  Therefore, it is very important that you select valid sysObjectID_match, sysDescr_match, and scanning function for your devices.', 'mactrack'); ?></strong>
			<br>
		</td>
	</tr><?php
...
function mactrack_get_device_types(&$sql_where, $rows, $apply_limits = true)
{
    ...
    if ('-1' == get_request_var('enabled')) {
        // Show all items
    } else {
        $sql_where .= ('' != $sql_where ? ' AND ' : ' WHERE ').'(mtdt.disabled='.(1 == get_request_var('enabled') ? "''" : "'on'").')';
    }
...
function mactrack_device_type()
{
    ...
    if (cacti_sizeof($device_types)) {
        ...
        echo '<tr><td colspan="'.$columns.'"><em>'.__('No Mactrack Device Types Found', 'mactrack').'</em></td></tr>';
    }
...
function mactrack_device_type_filter()
{
		...
		<form id='mactrack'>
			<table class='filterTable'>
				<tr>
					...
					<td>
						<?php echo __('Device Types', 'mactrack'); ?>
					</td>
					<td>
    ...
    ?>
					...
					<td>
						<span class='nowrap'>
							<button type='submit' id='go' class='ui-button ui-corner-all ui-widget ui-state-active' title='<?php echo __esc('Submit Query'); ?>'><?php echo __esc('Go'); ?></button>
							<button type='button' id='clear' class='ui-button ui-corner-all ui-widget' title='<?php echo __esc('Clear Filtered Results'); ?>'><?php echo __esc('Clear'); ?></button>
							<button type='button' id='scan' class='ui-button ui-corner-all ui-widget' title='<?php echo __esc('Scan Active Devices for Unknown Device Types'); ?>'><?php echo __esc('Rescan'); ?></button>
							<button type='button' id='import' class='ui-button ui-corner-all ui-widget' title='<?php echo __esc('Import Device Types from a CSV File'); ?>'><?php echo __esc('Import'); ?></button>
							<button type='button' id='export' class='ui-button ui-corner-all ui-widget' title='<?php echo __esc('Export Device Types to Share with Others'); ?>'><?php echo __esc('Export'); ?></button>
						</span>
					</td>
					<td>
    ...
    ?>
					...
					<td>
						<select id='type_id' onChange='applyFilter()'>
							<option value='-1'<?php if ('-1' == get_request_var('type_id')) {
							    echo ' selected';
							}?>><?php echo __('All', 'mactrack'); ?></option>
							<option value='1'<?php if ('1' == get_request_var('type_id')) {
							    echo ' selected';
							}?>><?php echo __('Switch/Hub', 'mactrack'); ?></option>
							<option value='2'<?php if ('2' == get_request_var('type_id')) {
							    echo ' selected';
							}?>><?php echo __('Switch/Router', 'mactrack'); ?></option>
							<option value='3'<?php if ('3' == get_request_var('type_id')) {
							    echo ' selected';
							}?>><?php echo __('Router', 'mactrack'); ?></option>
						</select>
					</td>
					<td>
						<?php echo __('Enabled', 'mactrack'); ?>
					</td>
					<td>
						<select id='enabled' onChange='applyFilter()'>
							<option value='-1'<?php if ('-1' == get_request_var('enabled')) {
							    echo ' selected';
							}?>><?php echo __('All', 'mactrack'); ?></option>
							<option value='1'<?php if ('1' == get_request_var('enabled')) {
							    echo ' selected';
							}?>><?php echo __('Yes', 'mactrack'); ?></option>
							<option value='2'<?php if ('2' == get_request_var('enabled')) {
							    echo ' selected';
							}?>><?php echo __('No', 'mactrack'); ?></option>
						</select>
					</td>
