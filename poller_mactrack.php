...
if ('disabled' == $collect_frequency) {
    echo 'WARNING: Mactrack scanning frequency is disabled, exiting. You have to enable it in settings.'.PHP_EOL;
...
if ($running_processes) {
    ...
    if (time() < (strtotime($start_date) + $max_run_duration) && !$force) {
        echo 'NOTE: Mactrack currently running and max run duration not eclipsed.'.PHP_EOL;
...
if ($site_id > 0) {
    ...
    if ($force || $collect_frequency > 0) {
        ...
        */
        ...
        if ($time_till_next_run < 0 || true == $force) {
...
function clear_old_processes($site_id)
{
    ...
    $old_procs = db_fetch_assoc_prepared(
        'SELECT mtp.*
...
function collect_mactrack_data($start, $site_id = 0)
{
    ...
    if ($site_id > 0) {
        $device_ids = db_fetch_assoc_prepared(
            'SELECT device_id
    ...
    if ($total_devices) {
        // grab arpwatch data
        if ('on' == read_config_option('mt_arpwatch')) {
        ...
        while ($processes_running > 0 && !$exit_mactrack) {
            ...
            $devices_running = db_fetch_cell("SELECT GROUP_CONCAT(CAST(`device_id` as CHAR) SEPARATOR ', ')
				FROM mac_track_processes");
        ...
        if (('on' == read_config_option('mt_arpwatch')) && cacti_sizeof($mac_ip_dns)) {
                ...
                foreach ($ports as $port) {
                    if (isset($mac_ip_dns[$port['mac_address']])) {
                        if ('' != $mac_ip_dns[$port['mac_address']]['dns']) {
                            db_execute_prepared(
                                'UPDATE mac_track_temp_ports
								SET updated = 1, ip_address = ?, dns_hostname = ?
								WHERE site_id = ?
								AND device_id = ?
								AND mac_address = ?',
                                [
        ...
        if ($debug) {
            ...
            $errors_tp = db_fetch_cell('SELECT COUNT(*) FROM mac_track_temp_ports WHERE mac_address like "% %"');
            $errors_arp = db_fetch_cell('SELECT COUNT(*) FROM mac_track_ips WHERE mac_address like "% %"');
            mactrack_debug("The Temp Ports table had {$errors_tp} mac tracks with spaces");
        ...
        while (!$exit_mactrack) {
            ...
            if (0 == cacti_sizeof($resolver_running)) {
                break;
            }
        ...
        if (is_array($ip_ranges)) {
            foreach ($ip_ranges as $ip_range) {
                $range_record = db_fetch_row_prepared(
                    'SELECT *
                ...
                if (0 == cacti_sizeof($range_record)) {
                    db_execute_prepared(
                        'REPLACE INTO `mac_track_ip_ranges`
        ...
        $ips = array_rekey(
            db_fetch_assoc_prepared(
                'SELECT site_id, COUNT(ip_address) AS total_ips
				FROM mac_track_ips
				WHERE scan_date = ?
				GROUP BY site_id',
                [$scan_date]
            ...
            foreach ($macwatches as $record) {
                ...
                if (cacti_sizeof($found)) {
                    ...
                    if (!$record['discovered'] || $record['notify_schedule'] >= '2') {
                        ...
                        if ($mail) {
                            mactrack_mail($record['email_addresses'], $from, $fromname, $subject, $message);
                        }
                    }

                    // update the the correct information
                    db_execute_prepared(
                        'UPDATE mac_track_macwatch
						SET discovered = 1, date_last_seen=NOW()'
                        .(0 == strtotime($record['date_first_seen']) ? ', date_first_seen=NOW()' : '').'
						WHERE mac_address = ?',
                        [$record['mac_address']]
                    );
                }
        ...
        if ('disabled' != $mac_auth_frequency) {
...
function display_version()
{
    ...
    echo 'Mactrack Master Poller, Version '.$info['version'].', '.COPYRIGHT_YEARS.PHP_EOL;
}
...
function display_help()
{
    ...
    echo PHP_EOL;
    echo 'usage: poller_mactrack.php [-sid=site_id] [--web] [--force] [--debug]'.PHP_EOL.PHP_EOL;
    echo '-sid=site_id  - The mac_track_sites site_id to scan'.PHP_EOL;
    echo '-w | --web    - Display output suitable for the web'.PHP_EOL;
    echo '-f | --force  - Force the execution of a collection process'.PHP_EOL;
